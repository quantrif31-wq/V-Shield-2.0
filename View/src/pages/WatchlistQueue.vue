<template>
    <div class="page-container ops-page animate-in">
        ...
        <section v-if="activeTab === 'matches'" class="ops-panel">
            ...
            <div v-else class="table-container">
                <table class="data-table">
                    <thead>
                        <tr><th>Entry</th><th>Severity</th><th>Visitor</th><th>Matched At</th><th>Status</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                        <tr v-for="m in filteredMatches" :key="m.watchlistMatchId">
                            ...
                            <td>{{ m.visit?.visitorName || '—' }}</td>
                            <td>{{ formatDate(m.matchedAtUtc) }}</td>
                            <td><span class="soft-chip" :class="matchStatusClass(m.status)">{{ m.status }}</span></td>
                            <td>
        ...
        <section v-if="activeTab === 'entries'" class="ops-panel">
            ...
            <div v-else-if="entries.length === 0" class="empty-card">No watchlist entries.</div>
            <div v-else class="table-container">
                <table class="data-table">
                    <thead>
                    ...
                    <tbody>
                        <tr v-for="e in entries" :key="e.watchlistEntryId">
                            <td>{{ e.displayName }}</td>
                            <td>{{ e.entityType }}</td>
                            <td>{{ e.identifier || '—' }}</td>
...
const filteredMatches = computed(() => {
    ...
    return matches.value.filter(m => m.status.toLowerCase().includes(filterStatus.value.toLowerCase()))
})
...
async function loadMatches() {
    const res = await enterpriseApi.getWatchlistMatches({ page: page.value, pageSize: 25 })
    const data = res.data || {}
