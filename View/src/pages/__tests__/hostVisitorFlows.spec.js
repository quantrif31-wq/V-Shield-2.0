...
describe('HostVisitorPage flows', () => {
  ...
  it('opens the form-templates modal from the header button', async () => {
    ...
    await wrapper.findAll('button').find((b) => b.text() === 'Biểu mẫu').trigger('click')
    await flushPromises()
    const closeX = document.body.querySelector('.modal-panel .btn-close')
    closeX.click()
    await flushPromises()
    expect(wrapper.vm.showFormTemplates).toBe(false)
    await wrapper.findAll('button').find((b) => b.text() === 'Biểu mẫu').trigger('click')
    await flushPromises()
    const footerClose = [...document.body.querySelectorAll('button')].find((b) => b.textContent.trim() === 'Đóng')
    footerClose.click()
    await flushPromises()
    expect(wrapper.vm.showFormTemplates).toBe(false)
  })
  ...
  it('issues a credential through the QR button and modal', async () => {
    ...
    const modalInputs = document.body.querySelectorAll('.modal-panel input[type="datetime-local"]')
    modalInputs[0].value = '2026-08-10T08:00'
    modalInputs[0].dispatchEvent(new Event('input'))
    modalInputs[1].value = '2026-08-12T08:00'
    ...
    wrapper.vm.issueCredential({ visitId: 30, visitorName: 'X' })
    await nextTick()
    const footerClose = [...document.body.querySelectorAll('button')].find((b) => b.textContent.trim() === 'Đóng')
    footerClose.click()
    await flushPromises()
  ...
  it('opens and closes the parking modal via the button', async () => {
    ...
    const areaSelect = document.body.querySelector('.modal-panel select')
    areaSelect.value = '5'
    areaSelect.dispatchEvent(new Event('change'))
    const pInputs = document.body.querySelectorAll('.modal-panel input')
    const dateInputs = [...pInputs].filter((i) => i.type === 'datetime-local')
    dateInputs[0].value = '2026-08-10T09:00'
    dateInputs[0].dispatchEvent(new Event('input'))
    dateInputs[1].value = '2026-08-10T17:00'
    dateInputs[1].dispatchEvent(new Event('input'))
    const plateInput = [...pInputs].find((i) => i.type === 'text' && (i.placeholder || '').includes('29A'))
    plateInput.value = '29A-777'
    plateInput.dispatchEvent(new Event('input'))
    expect(wrapper.vm.parkingForm.areaId).toBe(5)
    expect(wrapper.vm.parkingForm.plate).toBe('29A-777')
    const closeX = document.body.querySelector('.modal-panel .btn-close')
    closeX.click()
    await flushPromises()
  ...
  it('fills the create-invitation form fields and cancels', async () => {
    enterpriseApi.getFormTemplates.mockResolvedValue({ data: { items: [{ formTemplateId: 9, templateName: 'Mẫu NDA' }] } })
    const wrapper = mount(HostVisitorPage)
    await flushPromises()
    await wrapper.findAll('button').find((b) => b.text() === 'Lời mời mới').trigger('click')
    await nextTick()
    expect(wrapper.vm.showForm).toBe(true)

    const txt = (sel) => wrapper.find(sel)
    await txt('input[placeholder="Họ và tên"]').setValue('Khách Form')
    const inputs = wrapper.findAll('.form-control')
    const phone = inputs.find((i) => i.attributes('type') === 'text' && i.element.placeholder !== 'Họ và tên')
    await phone.setValue('0900')
    const email = inputs.find((i) => i.attributes('type') === 'email')
    await email.setValue('a@b.com')
    const dates = wrapper.findAll('input[type="datetime-local"]')
    await dates[0].setValue('2026-08-10T09:00')
    await dates[1].setValue('2026-08-10T17:00')
    expect(wrapper.vm.form.name).toBe('Khách Form')
    expect(wrapper.vm.form.expectedOut).toContain('2026-08-10')

    const checks = wrapper.findAll('input[type="checkbox"]')
    for (const c of checks) await c.setValue()
    await nextTick()
    expect(wrapper.vm.form.parkingRequired).toBe(true)
    expect(wrapper.vm.form.ndaRequired).toBe(true)
    expect(wrapper.find('input[placeholder="vd. 29A-12345"]').exists()).toBe(true)
    await wrapper.find('input[placeholder="vd. 29A-12345"]').setValue('29A-123')

    wrapper.vm.sites = [{ siteId: 1, name: 'Sảnh A' }]
    wrapper.vm.formExtra = { selectedNdaTemplateId: null, ndaTemplate: true }
    await nextTick()
    const selects = wrapper.findAll('select.form-control')
    await selects[0].setValue('1')
    expect(wrapper.vm.form.siteId).toBe(1)
    await selects[1].setValue(String(9))
    expect(wrapper.vm.formExtra.selectedNdaTemplateId).toBe(9)
  ...
  it('searches invitations through the search bar', async () => {
    enterpriseApi.getVisits.mockResolvedValue({ data: { items: [
      { visitId: 1, visitorName: 'Tuấn Anh' },
      { visitId: 2, visitorName: 'Lan Hương' },
    ] } })
    const wrapper = mount(HostVisitorPage)
    await flushPromises()
    await wrapper.find('input[placeholder="Tìm khách..."]').setValue('lan')
    expect(wrapper.vm.filteredInvitations.length).toBe(1)
  })
})
