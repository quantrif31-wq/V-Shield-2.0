<template>
    <div class="page-container identity-management animate-in">
        <div class="page-header-bar">
            <div>
                <span class="panel-kicker">Enterprise Identity</span>
                <h1 class="page-title">Identity Management</h1>
            </div>
            <div class="header-actions">
                <button type="button" class="btn btn-sm btn-warning" @click="showOffboard = true">Offboard Employee</button>
                <button type="button" class="btn btn-primary" :disabled="loading" @click="loadAll">Refresh</button>
            </div>
        </div>

        <section class="metric-grid">
            <article class="metric-tile">
                <span class="metric-label">IdP Providers</span>
                <strong class="metric-value">{{ overview.providers }}</strong>
                <span class="metric-note">{{ overview.enabledProviders }} enabled</span>
            </article>
            <article class="metric-tile">
                <span class="metric-label">Identity Mappings</span>
                <strong class="metric-value">{{ overview.mappings }}</strong>
                <span class="metric-note">{{ overview.activeMappings }} active</span>
            </article>
            <article class="metric-tile">
                <span class="metric-label">Active Employees</span>
                <strong class="metric-value">{{ overview.activeEmployees }}</strong>
                <span class="metric-note">{{ overview.suspendedEmployees }} suspended</span>
            </article>
            <article class="metric-tile">
                <span class="metric-label">Terminated</span>
                <strong class="metric-value">{{ overview.terminatedEmployees }}</strong>
                <span class="metric-note">{{ overview.recertificationCampaigns }} campaigns</span>
            </article>
        </section>

        <section class="workspace-tabs">
            <button type="button" :class="{ active: tab === 'providers' }" @click="tab = 'providers'">Identity Providers</button>
            <button type="button" :class="{ active: tab === 'import' }" @click="tab = 'import'">Import Users / Groups</button>
            <button type="button" :class="{ active: tab === 'mappings' }" @click="tab = 'mappings'; loadMappings()">Mappings</button>
        </section>

        <section v-if="tab === 'providers'" class="soc-section">
            <div class="section-toolbar">
                <h2>External Identity Providers</h2>
                <button type="button" class="btn btn-primary btn-sm" @click="openAddProvider">Add Provider</button>
            </div>
            <div v-if="loading" class="empty-card">Loading providers...</div>
            <div v-else-if="!providers.length" class="empty-card">No identity providers configured.</div>
            <div v-else class="provider-list">
                <div v-for="p in providers" :key="p.externalIdentityProviderId" class="provider-row" @click="editProvider(p)">
                    ...
                    <div class="provider-badges">
                        <span class="badge" :class="p.isEnabled ? 'badge-green' : 'badge-gray'">{{ p.isEnabled ? 'Enabled' : 'Disabled' }}</span>
        ...
        <section v-if="tab === 'import'" class="soc-section">
            <div class="import-grid">
                ...
                <div class="import-panel">
                    ...
                    <div class="form-grid single">
                        <label>
                            Group rows (one per line: Code, Name)
                            <textarea v-model="importGroup.raw" rows="6" placeholder="SEC-GUARD, Security Guard Team"></textarea>
                        </label>
                        <button type="button" class="btn btn-primary btn-sm" :disabled="!importGroup.raw" @click="doImportGroups">Import Groups</button>
                    </div>
        ...
        <section v-if="tab === 'mappings'" class="soc-section">
            ...
            <div v-if="mappings.length === 0" class="empty-card">No mappings found.</div>
            <div v-else class="mapping-list">
                <div v-for="m in mappings" :key="m.externalIdentityMappingId" class="mapping-row">
                    <div class="mapping-info">
                        ...
                        <span class="mapping-meta">Provider: {{ m.externalIdentityProviderId }} &middot; Sub: {{ m.externalSubject }}</span>
                    </div>
                    <span class="badge" :class="m.isActive ? 'badge-green' : 'badge-gray'">{{ m.isActive ? 'Active' : 'Inactive' }}</span>
                </div>
            </div>
        </section>
            ...
            <div class="modal-content modal-lg">
                ...
                <div class="modal-body">
                    ...
                    <div v-if="challengeUrl" class="challenge-box">
                        <strong>OIDC Challenge URL</strong>
                        <code class="challenge-url">{{ challengeUrl }}</code>
                        <button type="button" class="btn btn-sm btn-secondary" @click="copyChallenge">Copy</button>
                    </div>
                </div>
                <div class="modal-footer">
                    <button v-if="editingProvider && form.protocol === 'OIDC' && form.clientId" type="button" class="btn btn-secondary" @click="buildChallenge">Test OIDC Challenge</button>
                    <button type="button" class="btn btn-primary" :disabled="!form.name || !form.authority" @click="saveProvider">{{ editingProvider ? 'Update' : 'Create' }}</button>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-grid single">
                        ...
                        <label>
                            Reason
                            <textarea v-model="offboardForm.reason" rows="3" placeholder="Termination reason..."></textarea>
                        </label>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-warning" :disabled="!offboardForm.employeeId" @click="doOffboard">Confirm Offboard</button>
                    <button type="button" class="btn btn-secondary" @click="showOffboard = false">Cancel</button>
                </div>
            </div>
        </div>
    </div>
...
async function buildChallenge() {
    ...
    try {
        ...
        alert(err.response?.data?.message || 'Failed to build challenge')
    }
}
...
async function doOffboard() {
    ...
    try {
        ...
        alert(`Offboarded employee ${offboardForm.employeeId}. Token version: ${res.data.tokenVersion}`)
        showOffboard.value = false
        offboardForm.employeeId = null
        offboardForm.reason = ''
        await loadOverview()
    } catch (err) {
        alert(err.response?.data?.message || 'Offboard failed')
    }
}
...
.provider-row { display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-radius: 12px; border: 1px solid var(--border-soft); background: var(--surface); cursor: pointer; transition: border-color .15s; }
.provider-row:hover { border-color: var(--primary); }
...
.import-ok { color: #22c55e; background: rgba(34,197,94,.08); }
.import-skip { color: #f59e0b; background: rgba(245,158,11,.08); }
...
.challenge-box { margin-top: 12px; padding: 14px; background: rgba(59,130,246,.08); border-radius: 10px; display: flex; flex-direction: column; gap: 8px; }
