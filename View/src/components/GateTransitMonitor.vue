<template>
  <div class="page">
    <div class="topbar" :class="{ compact: topbarCompact }">
      <div class="topbar-main">
        ...
        <p v-show="!topbarCompact" class="topbar-desc">
          2 làn - QR + Bi?n s? - dùng employeeId t? QR d? g?i API thông hành cu
        </p>
      </div>
      <button
        type="button"
      ...
      >
        {{ topbarCompact ? "M? r?ng mô t?" : "Thu g?n mô t?" }}
      </button>
    </div>
              ...
              <div class="cam-preview" :class="`state-${cameraVisualState('qr', lane)}`">
                ...
                <div class="cam-preview-toolbar">
                  <button
                    ...
                    :aria-label="
                      lane.loading ? 'Ðang x? lý' : lane.qr.cameraRunning ? 'Ð?c l?i QR' : 'Ð?c QR'
                    "
    ...
    <div class="ops-dock" role="toolbar" aria-label="Ði?u khi?n nhanh">
        ...
        <div class="ops-dock-center">
          ...
          >
            ...
            <div class="lane-action-btns" role="group" :aria-label="`Thao tác ${lane.name}`">
              ...
              >
                {{
                  lane.loading ? "..." : laneAnyRunning(lane) ? "Ð?c l?i c? 2" : "Ð?c c? 2"
                }}
              </button>
    ...
    <div v-show="opsDrawerOpen" class="ops-drawer-root" :aria-hidden="!opsDrawerOpen">
      ...
      <aside class="ops-drawer-panel" role="dialog" aria-modal="true" aria-label="Cài d?t c?ng" @click.stop>
        <header class="ops-drawer-head">
          <h2 class="ops-drawer-title">Cài d?t</h2>
          <button type="button" class="ops-drawer-close" aria-label="Ðóng" @click="closeOpsDrawer">×</button>
        </header>
        ...
        <div class="ops-drawer-body">
          <div class="drawer-settings-panel">
            ...
            <p class="drawer-settings-hint">
              B?t/t?t backend tuong ?ng. URL stream l?y theo tab làn dang ch?n (QR / Plate). D?ch v? QR là m?t
              process dùng chung cho c? hai làn.
            </p>
          ...
          >
            <div class="lane-head">
              ...
              <div class="lane-final-status" :class="isLaneReady(activeOpsLane) ? 'ok' : 'wait'">
                {{ isLaneReady(activeOpsLane) ? "S?N SÀNG XÁC NH?N" : "ÐANG X? LÝ" }}
              </div>
            </div>
            ...
            <div class="summary-bar">
              <div class="summary-item">
                <span class="label">Ngu?i dùng</span>
            ...
            <div class="drawer-secondary-actions">
              ...
              >
                {{ activeOpsLane.loading ? "Ðang x? lý..." : "Preview" }}
              </button>
              <button
                type="button"
              ...
              >
                {{ activeOpsLane.loading ? "Ðang x? lý..." : "T?t" }}
              </button>
            </div>
          </section>
        </div>
...
},
{
  ...
  lane.qr.resultTimer = setInterval(async () => {
    ...
    if (lane.qr.scanRequested && !lane.qr.sessionLocked) {
      lane.qr.message = lane.qr.message || "Ðang quét QR..."
    }
...
const qr = lane.qr
  ...
  return // ðŸ”¥ STOP scan
    ...
    qrStateText(qr) {
      ...
      if (!qr.activeSessionPayload) return "ÐANG QUÉT"
      if (qr.activeSessionVerifyState === "waiting") return "ÐANG XÁC TH?C"
      if (qr.activeSessionVerifyState === "success") return "ÐÃ NH?N DI?N"
      if (qr.activeSessionVerifyState === "expired") return "H?T H?N"
      if (qr.activeSessionVerifyState === "invalid") return "KHÔNG H?P L?"
    ...
    hasInvalidHint(message) {
      ...
      return (
        normalized.includes("không h?p l?") ||
        normalized.includes("th?t b?i") ||
        normalized.includes("h?t h?n") ||
        normalized.includes("timeout") ||
        normalized.includes("quá th?i gian")
      )
    },
    ...
    async attachQrHlsPreview(laneId, qr) {
      ...
      try {
        ...
        if (!Hls?.isSupported?.()) {
          throw new Error("Trình duy?t này không h? tr? HLS.")
        }
    ...
    platePreviewStatusText(plate) {
      ...
      if (plate.lockedSnapshot || plate.lockedPlateCrop) return "?nh dã ch?p"
    ...
    checkQrSessionExpiry(lane) {
      ...
      if (diff >= qr.absenceThresholdMs) {
        this.clearQrState(qr)
        qr.message = "Mã dã bi?n m?t kh?i camera, phiên cu dã t? d?ng k?t thúc. Ðang ch? mã m?i."
    ...
    async captureAndDecodeQr(lane) {
      ...
      try {
        ...
        qr.activeSessionVerifyMessage = "Ðã phát hi?n mã m?i, dang xác th?c..."
        qr.lastSeenAt = now
        qr.lastUpdate = this.nowText()
        qr.message = "Ðang xác th?c QR..."
        ...
        if (result?.success) {
          ...
          qr.activeSessionVerifyMessage = result.message || "Xác th?c QR thành công."
  ...
  qr.employeeName = result?.data?.fullName || result?.data?.visitorName || ""
        ...
        if (message.includes("ã h?t h?n") || message.includes("chua ?n hi!u l?c")) {
          qr.activeSessionVerifyState = "expired"
        } else if (message.includes("không h?p l?")) {
          qr.activeSessionVerifyState = "invalid"
        } else {
          qr.activeSessionVerifyState = "failed"
        }

        qr.activeSessionVerifyMessage = message || "Xác th?c th?t b?i."
        qr.verifyMessage = message || "Xác th?c th?t b?i."
        qr.alert = true
        qr.message = qr.verifyMessage
      } catch (e) {
        console.warn("Decode QR frame error:", e)
        qr.verifyMessage = "Không d?c du?c frame t? IP camera. Ki?m tra CORS, mixed content ho?c URL stream."
...
if (safePayload.startsWith("EMP:")) {
  ...
  if (!lane.qr.cameraIp.trim() && !lane.plate.cameraIp.trim()) {
    alert("Vui lòng nh?p ít nh?t 1 URL camera")
    return
  }
  ...
  try {
    ...
    if (lane.qr.viewUrl) {
      ...
      this.mountPreview(lane.qr, lane.qr.viewUrl)
      lane.qr.message = "Ðã reload preview QR"
    ...
    if (lane.plate.viewUrl) {
      ...
      this.mountPreview(lane.plate, lane.plate.viewUrl)
      lane.plate.message = "Ðã reload preview Plate"
    }
    ...
    async readAllLane(lane) {
      if (!lane.qr.cameraIp.trim() || !lane.plate.cameraIp.trim()) {
        alert("Vui lòng nh?p d? URL QR và Plate")
        return
      }
...
this.clearQrState(lane.qr)
  ...
  lane.qr.message = "Ðang scan QR t? Python"
...
lane.qr.scanRequested = true
        ...
        if (!lane.plate.cameraRunning) {
          ...
          if (!resPlate?.success) {
            alert(resPlate?.message || "Không th? kh?i t?o Plate")
            return
          }
          lane.plate.cameraRunning = true
          lane.plate.sessionId = Number(resPlate.session_id || 0)
          lane.plate.lastAppliedSessionId = lane.plate.sessionId
          lane.plate.message = resPlate.message || "Khxi t?o Plate thành công"
        } else {
          const resPlate = await lane.plateApi.resetCameraState()
          lane.plate.message = resPlate?.message || "Ðã reset Plate"
  ...
  try {
    ...
    lane.qr.cameraRunning = true
    lane.qr.sessionLocked = false
    lane.qr.message = "Ðang scan l?i QR..."
    ...
    async retryPlate(lane) {
      ...
      try {
        ...
        if (!lane.plate.cameraRunning) {
          ...
          if (!res?.success) {
            alert(res?.message || "Không th? kh?i t?o Plate")
            return
          }
          lane.plate.cameraRunning = true
          lane.plate.sessionId = Number(res.session_id || 0)
          lane.plate.lastAppliedSessionId = lane.plate.sessionId
          lane.plate.message = res.message || "Khxi t?o Plate thành công"
        } else {
          const res = await lane.plateApi.resetCameraState()
          lane.plate.message = res?.message || "Ðã reset Plate"
  ...
  try {
    ...
    try {
      ...
      lane.plate.message = resPlate?.message || "Ðã t?t Plate"
    } catch (e) {
  ...
  if (!licensePlate) {
    alert(`${lane.name}: chua có bi?n s?`)
    return
  }

  if (!isGuest && !employeeId) {
    alert(`${lane.name}: chua có Employee ID`)
    return
  }

  if (isGuest && !visitorDetailId) {
    alert(`${lane.name}: chua có VisitorDetailId`)
    return
  }
  ...
  payload.QrPayload = lane.qr.qrPayload || lane.qr.activeSessionPayload || ""
    ...
    if (data?.success) {
      ...
      alert(`${lane.name}: ${data?.message || "X? lý th?t b?i"}`)
    }
  } catch (error) {
    const message =
      error?.response?.data?.message ||
      error?.message ||
      "Không g?i du?c API Gate"
...
selectCamera(cam, lane, type) {
  if (!cam.urlView) {
    alert("Camera chua có UrlView. Hãy reload go2rtc tru?c")
    return
  }
