...
public class PlateCameraController : ControllerBase
{
    ...
    public PlateCameraController(IHttpClientFactory httpClientFactory, IConfiguration configuration)
    {
        _httpClientFactory = httpClientFactory;
        _serviceBaseUrl = ResolveServiceBaseUrl(
            configuration["AiServices:PlateBaseUrl"],
    ...
    private async Task<IActionResult> ProxyGetAsync(string relativePath)
    {
        ...
        {
            return StatusCode(503, new { message = $"Khong the ket noi toi bien so service: {ex.Message}" });
        }
    }
